---
pageType: concept
id: concept.uncensored-ai-models
title: Uncensored AI Models
provenanceMode: unsafe-local
sourcePath: /tmp/uncensored-ai-models.md
unsafeLocalConfiguredPath: /media/racoony-wiki
unsafeLocalRelativePath: /tmp/uncensored-ai-models.md
updatedAt: '2026-04-25T06:54:12.377559+00:00'
sourceIds:
- githubcom
sources:
- sourceId: githubcom
  sourceType: web
  sourcePath: https://github.com/comfyanonymous/ComfyUI.git
  title: github.com
claims:
...
Overview of approaches, tools, and hardware requirements for running uncensored or de-aligned AI models — covering both text LLMs and image generation.

## Text LLMs (Uncensored / Abliterated)
...
### Two Paths to Uncensored LLMs

1. **Pre-built uncensored models** — community has already stripped guardrails. Download and run.
2. **Abliterate your own** — use OBLITERATUS to surgically remove refusal directions from any open-weight model's weights. No retraining needed.

### OBLITERATUS

CLI tool for removing refusal behaviors from open-weight LLMs. Uses mechanistic interpretability techniques (diff-in-means, SVD, whitened SVD, LEACE, SAE decomposition, etc.) to excise refusal directions while preserving reasoning.
...
### Key Pre-Built Uncensored Models

#### Efficiency Tier (≤8B, laptop-friendly)
...
#### Sweet Spot (12-30B, mid-range GPU)
...
### Famous Uncensored Model Series
...
## Image Generation (No "Abliteration" Needed)

Image models (diffusion models) don't have refusal mechanisms like LLMs. They just output pixels. "Uncensoring" means:
1. Disabling the safety checker (a separate classifier, trivially toggled off)
2. Using community models trained on unrestricted data

### Mac Setup Options

**Draw Things** (easiest): Mac App Store, one-click, native Apple Silicon. Supports SDXL, LoRAs, ControlNet. Toggle off safety checker in settings.

**ComfyUI** (powerful): Node-based workflow UI. More control, steeper learning curve.
```bash
...
**Automatic1111 WebUI**: Most popular but memory-hungry. Not recommended for 16GB Mac.

### Unrestricted SDXL Community Checkpoints
...
Drop `.safetensors` files into `ComfyUI/models/checkpoints/` or Draw Things model folder.

## Hardware: 16GB MacBook Pro (Apple Silicon)
...
All LLMs should be run with Q4_K_M quantization via llama.cpp (Metal acceleration) or Ollama.
