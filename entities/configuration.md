...
Runs commands inside a Docker container with [[entities/security|Security]] hardening (all capabilities dropped, no privilege escalation, PID limits).
...
**[[entities/security|Security]] hardening:**

- `--cap-drop ALL` with only `DAC_OVERRIDE`, `CHOWN`, `FOWNER` added back
- `--[[entities/security|Security]]-opt no-new-privileges`
- `--pids-limit 256`
...
[[entities/security|Security]] tradeoff:
...
Environment scrubbing (strips `*_API_KEY`, `*_TOKEN`, `*_SECRET`, `*_PASSWORD`, `*_CREDENTIAL`, `*_PASSWD`, `*_AUTH`) and the tool whitelist apply identically in both modes — switching mode does not change the [[entities/security|Security]] posture.
...
## [[entities/security|Security]][​](#[[entities/security|Security]] "Direct link to [[entities/security|Security]]")

Pre-execution [[entities/security|Security]] scanning and secret redaction:

```
[[entities/security|Security]]:
  redact_secrets: true           # Redact API key patterns in tool output and logs
  tirith_enabled: true           # Enable Tirith [[entities/security|Security]] scanning for terminal commands
